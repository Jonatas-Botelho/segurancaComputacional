...
public class PlayerShipActor extends DamageableActor  {
	...
	private final InputListener mInputListener = new InputListener() {
		...
		public boolean keyDown(InputEvent event, int keycode) {
			if((event.getKeyCode() == Keys.UP) || (event.getKeyCode() == Keys.W)  ) {
				mIsAccelerating = true;
				return true;
			}
			
			if((event.getKeyCode() == Keys.LEFT) || (event.getKeyCode() == Keys.A)) {
				mAngularSpeed = 5;
				return true;
			}
			
			if((event.getKeyCode() == Keys.RIGHT) || (event.getKeyCode() == Keys.D)) {
				mAngularSpeed = -5;
				return true;
			}
			
			if((event.getKeyCode() == Keys.SPACE) || (event.getKeyCode() == Keys.ALT_LEFT) ) {
				fireBullet();
				return true;
			}
